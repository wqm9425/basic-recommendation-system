        ...
        if os.path.exists('./X_train.npy'):
            ...
            for f in glob.glob('../images_background/*/*/*.png'):
                ...
                if key not in character.keys():
                    ...
                    counter += 1
                img = imread(f)
                img = resize(img, (105,105,1))
                D = np.concatenate((img,img), axis=2)
                img = np.concatenate((D,img), axis=2)
                X.append(img)
                y.append(character[key])
            X = np.array(X)
            y = np.array(y)
            X = X.reshape(-1,105,105,3)
            # split
...
class Trans:
    def build(self, input_shape, n_class):
        ...
        avg = GlobalAveragePooling2D()(vgg.layers[-2].output)
        out = Dense(n_class)(avg)
      
        self.trans = Model(inp, out)
        optimizer = Adam()
